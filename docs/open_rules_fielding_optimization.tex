...
\begin{enumerate}
    ...
    \item \textbf{One-inning phase.} For at most 0.25 seconds, test the ideal lower bound in which the
    ...
    \item \textbf{Two-inning phase.} With the secured $o_1$ and $e$ ceilings,
    minimize $o_2c_2+vc_v+q$ in two concurrent four-worker searches sharing
    the existing eight-worker computational envelope. One search is an
    ordinary proof-capable CP-SAT run; the second is a hint-seeded large-
    neighborhood search with a different seed. Retain the lexicographically
    better legal candidate and impose its non-worsening $o_2$ ceiling.
    \item \textbf{Tail-continuity phase.} Use the remaining nominal budget to
    minimize $vc_v+q$. Because a hint is not a quality guarantee, constrain
    this tail objective to be no larger than the incumbent value and accept a
    timed candidate only when its full $(o_1,e,o_2,v,q)$ vector is
    lexicographically no worse than that incumbent.
\end{enumerate}
...
excess positions, the dedicated two-inning tier, and the final distinct-position
and transition tail are all proven. Any legal incumbent lacking one of those
proofs is truthfully labeled \emph{FEASIBLE}.
The latter label concerns proof of objective optimality, not legality.

On the recorded macOS reference environment at commit \code{bf0f6b2}, twenty measured HFTB solves
were distributed across four fresh Python processes after one excluded warm-up
per process. All twenty preserved the proved fairness/preference prefix
$(4,210,0)$ and met the continuity floor $(5,0,9,20,21)$ lexicographically.
Seventeen returned $(5,0,9,19,20)$ and three returned $(5,0,9,20,20)$. Solver
latency had p95 5.056 seconds and maximum 5.057 seconds; the fairness/fallback
prefix had p95 0.127 seconds and maximum 0.132 seconds. Every result remained
truthfully \emph{FEASIBLE}.
