...
Players with the same woman/non-woman Boolean flag, eligible set, and explicit
preference set are interchangeable in the mathematical objectives. For each
such equivalence class, the implementation encodes every complete state
sequence as a base-$(K+1)$ integer and orders adjacent signatures
nondecreasingly. This removes name-permutation symmetry without removing a
distinct objective outcome. It is a search acceleration, not a softball rule.
...
The interactive default is a nominal ten-second multi-phase solve target measured
...
0.05--0.1 seconds can make observed elapsed time exceed ten seconds. The implementation uses the
...
At the September 2026 release gate, 95 Python tests and 12 Playwright browser
...
\begin{enumerate}
    ...
    optimization, both lineup views, inning selection, CSV download, invalid
...
The implementation, normative specification, test suite, and fairness
benchmark utility are maintained in the public repository at
...
\begin{itemize}
    ...
    Playwright workflows; and
    \item \code{python scripts/benchmark\_fairness.py --budgets 1 2 3}
    for repeated fairness-phase stress trials at selected roster sizes.
\end{itemize}
