...
The interactive default is a nominal five-second solver-phase target measured
...
and the implementation's 0.1-second minimum allocation for later phases can
make observed elapsed time exceed five seconds. The implementation uses the
...
\begin{enumerate}
    ...
    inning Bench run is forced by $t_p=6$. If that ideal slice is not found,
    minimize $o_1$ and preserve its incumbent.
    \item \textbf{Two-position phase.} Minimize $e$ with a decision strategy
    favoring unused position indicators. A proved value is fixed; an unproved
    incumbent becomes a non-worsening upper bound.
...
At the September 2026 release gate, 93 Python tests and 12 Playwright browser
