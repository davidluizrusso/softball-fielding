...
\begin{enumerate}
    ...
    candidate, retain the already legal fairness/preference incumbent and
    ...
    ceiling $J_C\leq J_C^{inc}$ so a later improvement in $o_1$ may
...
On the recorded macOS reference environment at commit \code{fa92000}, twenty measured HFTB solves
...
by an aggregate. Solver latency had p95 5.062 seconds and maximum 5.063 seconds;
the fairness/fallback prefix had p95 0.138 seconds and maximum 0.151 seconds.
Every result remained truthfully \emph{FEASIBLE}. Thus the continuity threshold
has a 90\%-minimum acceptance rate and achieved 95\% in the recorded reference;
it is a measured target for nondeterministic timed search, not a hard
feasibility promise.
...
This case study uses the original thirteen-player Team Red roster, all initially
available under locked Open rules. The current \code{team\_red\_roster.csv}
contains fourteen players, including Shaz, and updated preferences for Andrew
and David R. The original benchmark's explicit preferences are:
