...
For the current fourteen-player Team Red instance, the implementation produces
the provably fairest inning-count distribution, uses no fallback assignment,
and keeps every player at no more than two fielding positions. Automated model,
mutation, application-state, and mobile-browser tests establish the principal
claims.
...
This case study uses the current fourteen-player Team Red roster in
\code{team\_red\_roster.csv}, with every player initially available under
locked Open rules. Its explicit preferences are:
...
With $N=14$ and $K=10$, the instance contains $H=70$ fielding slots, for an
equal share of five innings per player. Dung is the only explicitly eligible
pitcher and therefore must pitch all seven innings. A playing-time spread of
two or less is impossible: with one player at seven innings, the other thirteen
players would each need at least five, requiring at least $72$ slots. A schedule
with one player at seven innings, eleven at five, and two at four uses exactly
\[
1(7)+11(5)+2(4)=70
\]
slots and attains spread three, proving the first fairness tier. To establish
the second tier, let $a$, $b$, $c$, and $d$ be the numbers of players receiving
four, five, six, and seven innings. Within a spread-three solution,
$a+b+c+d=14$, $4a+5b+6c+7d=70$, and therefore $a=c+2d$. Dung forces $d\geq1$.
The scaled deviation from five innings is $14(a+c+2d)=28c+56d$, which is
minimized by $(a,b,c,d)=(2,11,0,1)$. The resulting deviation is
\[
1|14(7)-70|+11|14(5)-70|+2|14(4)-70|=56.
\]
...
\begin{itemize}
    ...
    \item a representative nominal five-second solve has continuity vector
    $(3,0,15,20,20)$: three one-inning state runs, no excess field positions,
    15 exact two-inning state runs, 20 player-position incidences, and 20
    adjacent state transitions.
\end{itemize}

These measurements characterize a representative objective outcome, not a
unique name-by-name lineup or a proof of the final continuity tiers. Multiple
schedules can share the same lexicographic quality. The application returns one
solver-selected representative and uses it directly as the final defensive
schedule.
...
lineup. By contrast, a missing explicit catcher does not create infeasibility
because every player is eligible to catch by policy.

\section{Verification and Falsification Strategy}

Validation is organized around properties rather than visual plausibility.
At the September 2026 release gate, 144 Python tests and 16 Playwright browser
...
The Team Red case demonstrates the formulation on a real fourteen-player
...
target, and limits unnecessary state changes. The staged CP-SAT
